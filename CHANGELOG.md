...
All notable changes to FinRouter are documented here.
...
- **Plugin system**: `router.use(plugin)` installs reusable `FinRouterPlugin` extensions; duplicate installs are silently skipped
...
- **OpenAI-compatible middleware**: `createMiddleware(router)` from `finrouter/adapters` mounts FinRouter as an OpenAI-compatible `POST /v1/chat/completions` handler for Express and Fastify
- **Redis key store**: `RedisKeyStore` from `finrouter/adapters` — drop-in replacement for the in-memory key store for multi-instance deployments
- **Redis rate limiter**: `RedisRateLimiter` from `finrouter/adapters` — distributed token-bucket rate limiter using atomic Lua scripts
- **CLI**: `finrouter` binary with `validate-config`, `list-providers`, and `rotate-key` commands
...
- `StoredKey` is now an exported interface from `finrouter/security` — enables custom key store implementations
